...
struct CreateTaskView: View {
    ...
    var body: some View {
        NavigationStack {
            VStack {
                VStack(spacing: 20) {
                    TextFieldView(placeholder: "Enter the task name:",
                                  text: $taskViewModel.taskName,
                                  isFocused: _taskNameIsFocused)
                    
                    TextFieldView(placeholder: "Enter the task description:",
                                  text: $taskViewModel.taskDescription,
                                  isFocused: _taskDescriptionIsFocused)
                    
                    Button(action: {
                        let newTask = TaskModel(taskName: taskViewModel.taskName,
